...
object MovieRecommendationALSDataSet {
  ...
  def main(args: Array[String]) {
    ...
    val moviesNamesSchema = new StructType()
      .add(name="movieID", IntegerType, nullable = true)
      .add(name="movieTitle", StringType, nullable = true)

    // Create schema when reading u.data
    val moviesSchema = new StructType()
      .add(name ="userID", IntegerType, nullable = true)
      .add(name = "movieID", IntegerType, nullable = true)
      .add(name ="rating", IntegerType, nullable = true)
      .add(name ="timestamp", LongType, nullable = true)
    ...
    val users = Seq(userID).toDF(colNames = "userID")
    val recommendations = model.recommendForUserSubset(users, numItems = 10)
