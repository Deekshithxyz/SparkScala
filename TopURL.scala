...
object TopURL {
  ...
  def main(args: Array[String]) {
    ...
    val updLogsDF = logsDF.withColumn("eventTime",current_timestamp())

    val k = updLogsDF.groupBy(window(col("eventTime"),"30 Seconds","10 Seconds"),col("endpoint")).count().orderBy(asc("window"),desc("count"))

    val query = k.writeStream.option("truncate",false).outputMode(outputMode="complete").format(source="console").queryName(queryName="counts").start()
